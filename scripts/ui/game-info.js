import { DEFAULT_MAP_WIDTH, DEFAULT_MAP_HEIGHT, SEED } from '../entity/map.js';
...
export function createGameInfoUI() {
    ...
    <div class="info-content">Width: <span id="map-width">${DEFAULT_MAP_WIDTH}</span></div>
    <div class="info-content">Height: <span id="map-height">${DEFAULT_MAP_HEIGHT}</span></div>
  `;
    ...
    document.getElementById('copy-seed-btn').addEventListener('click', function () {
        ...
        navigator.clipboard
            ...
            .catch((error) =>
                console.error('Failed to copy the seed of map to the clipboard. Error:', error),
            );
